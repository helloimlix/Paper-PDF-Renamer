...
- Renames downloads with a configurable template, defaulting to `{title}.pdf`.
...
Supported tokens: `{title}`, `{date}`, `{year}`, `{authors}`, `{firstAuthor}`, `{source}`, and `{sourceId}`.
